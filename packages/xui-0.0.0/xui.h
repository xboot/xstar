#ifndef __XSTAR_XUI_H__
#define __XSTAR_XUI_H__
...
#define xui_stack_push(stk, val) \
	do { \
		if(stk.idx >= stk.capacity) { \
			stk.capacity = (stk.capacity == 0) ? 16 : (stk.capacity << 1); \
			stk.items = xos_mem_realloc(stk.items, (size_t)stk.capacity * sizeof(stk.items[0])); \
		} \
		(stk).items[(stk).idx] = (val); (stk).idx++; \
	} while(0)

#define xui_stack_pop(stk) \
	do { \
		if((stk).idx > 0) \
			(stk).idx--; \
	} while(0)

#define xui_timeout(ctx, ms) \
	({ \
		static ktime_t _t = { 0 }; \
		int _v = 0; \
		if(ktime_after((ctx)->stamp, ktime_add_ms(_t, (ms)))) \
		{ \
			_t = (ctx)->stamp; \
			_v = 1; \
		} \
...
struct xui_cmd_base_t {
	struct xui_cmd_header_t header;
};
...
static inline void xui_push_clip(struct xui_context_t * ctx, struct region_t * r)
{
	...
	xui_stack_push(ctx->clip_stack, clip);
}

static inline void xui_pop_clip(struct xui_context_t * ctx)
{
	xui_stack_pop(ctx->clip_stack);
}
...
{
	xui_stack_push(ctx->id_stack, xui_get_id(ctx, data, size));
}

static inline void xui_pop_id(struct xui_context_t * ctx)
{
	xui_stack_pop(ctx->id_stack);
}
