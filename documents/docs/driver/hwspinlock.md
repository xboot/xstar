...
硬件互斥锁接口。每个设备管理一段全局编号的锁空间（`base` ~ `base + nlock - 1`），设备树通过 `hwspinlock-base`、`hwspinlock-count` 属性描述。适用于 AMP 系统中处理器之间以及处理器与硬件模块之间的互斥同步——区别于 XOS 提供的自旋锁（`xos_spinlock_lock` 等），后者面向 SMP 内核态的互斥。`hwspinlock_desc_t` 描述符缓存了设备指针与锁偏移，适合高频持锁场景，避免每次操作都进行全局锁号查找。
