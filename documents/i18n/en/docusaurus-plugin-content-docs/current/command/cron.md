...
```
cron                             - list all jobs
cron add <name> <spec> <cmd>     - add a recurring job
cron oneshot <name> <spec> <cmd> - add a one-shot job
cron remove <name>               - remove a job
```

## Description

- `spec` is a standard 5-field cron expression; since it contains spaces, quote it: `"<minute> <hour> <day> <month> <week>"`
...
- Running `cron` with no arguments lists all jobs, each line showing `name`, `spec`, and the type (`periodic`/`oneshot`)
