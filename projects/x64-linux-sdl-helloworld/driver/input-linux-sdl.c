...
struct input_linux_sdl_pdata_t {
	void * ctx;
};
...
static int input_linux_sdl_ioctl(struct input_t * input, const char * cmd, void * arg)
{
	return -1;
}

static struct device_t * input_linux_sdl_probe(struct driver_t * drv, struct dtnode_t * n)
{
	struct input_linux_sdl_pdata_t * pdat;
	struct input_t * input;
	struct device_t * dev;
	void * ctx;

	ctx = linux_event_sdl_open();
	if(!ctx)
		return NULL;

	pdat = xos_mem_malloc(sizeof(struct input_linux_sdl_pdata_t));
	if(!pdat)
	{
		linux_event_sdl_close(ctx);
		return NULL;
	}
	...
	input->ioctl = input_linux_sdl_ioctl;
	input->priv = pdat;

	linux_event_sdl_set_key_callback(ctx, input,
			cb_key_down,
			cb_key_up);
	linux_event_sdl_set_mouse_callback(ctx, input,
			cb_mouse_down,
			cb_mouse_move,
			cb_mouse_up,
			cb_mouse_wheel);
	linux_event_sdl_set_touch_callback(ctx, input,
			cb_touch_begin,
			cb_touch_move,
			cb_touch_end);
	linux_event_sdl_set_joystick_callback(ctx, input,
	...
	{
		linux_event_sdl_close(input->priv);
...
static void input_linux_sdl_remove(struct device_t * dev)
{
...
static void input_linux_sdl_suspend(struct device_t * dev)
{
}

static void input_linux_sdl_resume(struct device_t * dev)
{
}

static struct driver_t input_linux_sdl = {
	.name		= "input-linux-sdl",
	.probe		= input_linux_sdl_probe,
	.remove		= input_linux_sdl_remove,
	.suspend	= input_linux_sdl_suspend,
	.resume		= input_linux_sdl_resume,
};

static void input_linux_sdl_driver_init(void)
{
	register_driver(&input_linux_sdl);
}

static void input_linux_sdl_driver_exit(void)
{
	unregister_driver(&input_linux_sdl);
}

driver_initcall(input_linux_sdl_driver_init);
driver_exitcall(input_linux_sdl_driver_exit);
