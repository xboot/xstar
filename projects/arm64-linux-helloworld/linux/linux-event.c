...
struct linux_event_context_t {
	struct linux_event_callback_t cb;
	struct pollfd * pfd;
	int npfd;
	char ** device;
	struct linux_event_buffer_t * buf;
	pthread_t thread;
	int xmax, ymax;
	int sensitivity;
	int xpos, ypos;
...
static int linux_event_open_device(struct linux_event_context_t * ctx, const char * dev)
{
	struct pollfd * new_pfd;
	struct input_id id;
	char ** new_device;
	struct linux_event_buffer_t * new_buf;
	char name[80];
	char location[80];
	char idstr[80];
	...
	new_buf = realloc(ctx->buf, sizeof(struct linux_event_buffer_t) * (ctx->npfd + 1));
	if(new_buf == NULL)
		return -1;
	ctx->buf = new_buf;
	memset(&ctx->buf[ctx->npfd], 0, sizeof(struct linux_event_buffer_t));

	ctx->pfd[ctx->npfd].fd = fd;
	ctx->pfd[ctx->npfd].events = POLLIN;
	ctx->device[ctx->npfd] = xos_strdup(dev);
	ctx->npfd++;
	return 0;
}

static int linux_event_close_device(struct linux_event_context_t * ctx, const char * dev)
{
	int i;
	for(i = 1; i < ctx->npfd; i++)
	{
		if(strcmp(ctx->device[i], dev) == 0)
		{
			...
			free(ctx->device[i]);
			memmove(ctx->buf + i, ctx->buf + i + 1, sizeof(struct linux_event_buffer_t) * count);
...
static int linux_event_read_notify(struct linux_event_context_t * ctx, const char * dirname, int nfd)
{
	...
	while(res >= sizeof(struct inotify_event))
	{
		...
		if(event->len)
		{
			...
			if(event->mask & IN_CREATE)
				linux_event_open_device(ctx, dev);
			else
				linux_event_close_device(ctx, dev);
		}
...
static int linux_event_scan_dir(struct linux_event_context_t * ctx, const char * dirname)
{
	struct dirent * de;
	char dev[PATH_MAX];
	char * name;
...
static void * linux_event_thread(void * arg)
{
	struct linux_event_context_t * ctx = (struct linux_event_context_t *)arg;
	...
	while(1)
	{
		...
		if(ctx->pfd[0].revents & POLLIN)
			linux_event_read_notify(ctx, "/dev/input", ctx->pfd[0].fd);
					...
					if(read(ctx->pfd[i].fd, &e, sizeof(struct input_event)) == sizeof(struct input_event))
					{
						...
						case EV_SYN:
							if(e.code == SYN_REPORT)
							{
								...
								if(ctx->buf[i].touch.press == 1)
								{
									if(cb->touch.begin)
										cb->touch.begin(cb->touch.device, ctx->buf[i].touch.x, ctx->buf[i].touch.y, 0);
									ctx->buf[i].touch.press = 0;
								}
								else if(ctx->buf[i].touch.press == -1)
								{
									if(cb->touch.end)
										cb->touch.end(cb->touch.device, ctx->buf[i].touch.x, ctx->buf[i].touch.y, 0);
									ctx->buf[i].touch.press = 0;
								}
								if(ctx->buf[i].touch.move != 0)
								{
									if(cb->touch.move)
										cb->touch.move(cb->touch.device, ctx->buf[i].touch.x, ctx->buf[i].touch.y, 0);
									ctx->buf[i].touch.move = 0;
								}
							}
							break;
						case EV_KEY:
							...
							else if(e.code >= BTN_DIGI && e.code < (BTN_DIGI + 16))
							{
								if((e.code == BTN_TOUCH) || (e.code == BTN_TOOL_FINGER))
									ctx->buf[i].touch.press = (e.value != 0) ? 1 : -1;
							}
						...
						case EV_ABS:
							...
							{
							case ABS_X:
								ctx->buf[i].touch.x = e.value;
								ctx->buf[i].touch.move = 1;
								break;
							case ABS_Y:
								ctx->buf[i].touch.y = e.value;
								ctx->buf[i].touch.move = 1;
								break;
							case ABS_MT_SLOT:
								break;
...
{
	struct linux_event_context_t * ctx;
	int i;

	ctx = malloc(sizeof(struct linux_event_context_t));
	if(!ctx)
		return NULL;

	memset(ctx, 0, sizeof(struct linux_event_context_t));
	ctx->npfd = 1;
	...
	if(linux_event_scan_dir(ctx, "/dev/input") < 0)
	{
	...
	if(pthread_create(&ctx->thread, NULL, linux_event_thread, ctx) < 0)
	{
		for(i = 0; i < ctx->npfd; i++)
			inotify_rm_watch(ctx->pfd[i].fd, IN_DELETE | IN_CREATE);
		free(ctx->pfd);
...
{
	struct linux_event_context_t * ctx = (struct linux_event_context_t *)context;
	int i;
