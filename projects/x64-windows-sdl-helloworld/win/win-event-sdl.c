...
static struct win_event_sdl_context_t {
	struct win_event_callback_t cb;
} __event_sdl = { 0 };
...
void win_event_sdl_set_key_callback(void * device,
		...
		void (*up)(void * device, unsigned int key))
{
	struct win_event_sdl_context_t * ctx = (struct win_event_sdl_context_t *)&__event_sdl;
	if(ctx)
	{
...
{
	struct win_event_sdl_context_t * ctx = (struct win_event_sdl_context_t *)&__event_sdl;
	struct win_event_callback_t * cb = &ctx->cb;
	SDL_Event e;
	unsigned int button;
	int width, height;
	...
	while(SDL_PollEvent(&e))
	{
		...
		case SDL_EVENT_MOUSE_BUTTON_DOWN:
			...
			if(cb->mouse.down && (button != 0x00))
				cb->mouse.down(cb->mouse.device, (int)e.button.x, (int)e.button.y, button);
			break;
		...
		case SDL_EVENT_MOUSE_BUTTON_UP:
			...
			if(cb->mouse.up && (button != 0x00))
				cb->mouse.up(cb->mouse.device, (int)e.button.x, (int)e.button.y, button);
			break;
