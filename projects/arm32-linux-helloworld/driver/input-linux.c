...
static int input_linux_ioctl(struct input_t * input, const char * cmd, void * arg)
{
	struct input_linux_pdata_t * pdat = (struct input_linux_pdata_t *)input->priv;
	int * p = arg;
	...
	case 0x431aa221: /* "mouse-set-range" */
		if(p)
		{
			linux_event_mouse_set_range(pdat->ctx, p[0], p[1]);
			return 0;
		}
		break;
	case 0xcd455615: /* "mouse-get-range" */
		if(p)
		{
			linux_event_mouse_get_range(pdat->ctx, &p[0], &p[1]);
			return 0;
		}
		break;
	case 0xe818d6df: /* "mouse-set-sensitivity" */
		if(p)
		{
			linux_event_mouse_set_sensitivity(pdat->ctx, XCLAMP(p[0], 1, 11));
			return 0;
		}
		break;
	case 0x40bfb1d3: /* "mouse-get-sensitivity" */
		if(p)
		{
			linux_event_mouse_get_sensitivity(pdat->ctx, &p[0]);
			return 0;
		}
		break;
	default:
...
static struct device_t * input_linux_probe(struct driver_t * drv, struct dtnode_t * n)
{
	struct input_linux_pdata_t * pdat;
	struct input_t * input;
	struct device_t * dev;
	void * ctx;

	ctx = linux_event_open();
	if(!ctx)
		return NULL;

	pdat = xos_mem_malloc(sizeof(struct input_linux_pdata_t));
	if(!pdat)
	{
		linux_event_close(ctx);
		return NULL;
	}
	...
	input->ioctl = input_linux_ioctl;
	input->priv = pdat;

	linux_event_mouse_set_range(pdat->ctx, dt_read_int(n, "pointer-range-x", 640), dt_read_int(n, "pointer-range-y", 480));
	linux_event_mouse_set_sensitivity(pdat->ctx, dt_read_int(n, "pointer-sensitivity", 6));
	linux_event_set_key_callback(pdat->ctx, input,
			cb_key_down,
			cb_key_up);
	linux_event_set_mouse_callback(pdat->ctx, input,
			cb_mouse_down,
			cb_mouse_move,
			cb_mouse_up,
			cb_mouse_wheel);
	linux_event_set_touch_callback(pdat->ctx, input,
			cb_touch_begin,
			cb_touch_move,
			cb_touch_end);
	linux_event_set_joystick_callback(pdat->ctx, input,
...
static void input_linux_remove(struct device_t * dev)
{
...
static void input_linux_suspend(struct device_t * dev)
{
}

static void input_linux_resume(struct device_t * dev)
{
}

static struct driver_t input_linux = {
	.name		= "input-linux",
	.probe		= input_linux_probe,
	.remove		= input_linux_remove,
	.suspend	= input_linux_suspend,
	.resume		= input_linux_resume,
};

static void input_linux_driver_init(void)
{
	register_driver(&input_linux);
}

static void input_linux_driver_exit(void)
{
	unregister_driver(&input_linux);
}

driver_initcall(input_linux_driver_init);
driver_exitcall(input_linux_driver_exit);
