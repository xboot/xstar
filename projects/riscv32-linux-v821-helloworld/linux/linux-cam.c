...
enum {
	LINUX_CAMERA_FORMAT_ARGB	= 0,
	LINUX_CAMERA_FORMAT_YUYV	= 1,
	LINUX_CAMERA_FORMAT_UYVY	= 2,
	LINUX_CAMERA_FORMAT_NV12	= 3,
	LINUX_CAMERA_FORMAT_NV21	= 4,
	LINUX_CAMERA_FORMAT_YU12	= 5,
	LINUX_CAMERA_FORMAT_YV12	= 6,
	LINUX_CAMERA_FORMAT_MJPG	= 7,
};

struct linux_cam_buf_t {
    void * start;
    size_t length;
};

struct linux_cam_context_t {
	int fd;
	int nbuf;
	struct linux_cam_buf_t * buf;
};
...
{
	struct linux_cam_context_t * ctx;
	...
	ctx = malloc(sizeof(struct linux_cam_context_t));
	if(!ctx)
		return NULL;
	...
	switch(*format)
	{
	case LINUX_CAMERA_FORMAT_ARGB:
		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_RGB32;
		break;
	case LINUX_CAMERA_FORMAT_YUYV:
		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;
		break;
	case LINUX_CAMERA_FORMAT_UYVY:
		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_UYVY;
		break;
	case LINUX_CAMERA_FORMAT_NV12:
		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_NV12;
		break;
	case LINUX_CAMERA_FORMAT_NV21:
		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_NV21;
		break;
	case LINUX_CAMERA_FORMAT_YU12:
		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUV420;
		break;
	case LINUX_CAMERA_FORMAT_YV12:
		fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YVU420;
		break;
	case LINUX_CAMERA_FORMAT_MJPG:
	...
	if(ioctl(ctx->fd, VIDIOC_G_FMT, &fmt) == 0)
	{
		...
		*height = fmt.fmt.pix.height;
		...
		case V4L2_PIX_FMT_RGB32:
			*format = LINUX_CAMERA_FORMAT_ARGB;
			break;
		case V4L2_PIX_FMT_YUYV:
			*format = LINUX_CAMERA_FORMAT_YUYV;
			break;
		case V4L2_PIX_FMT_UYVY:
			*format = LINUX_CAMERA_FORMAT_UYVY;
			break;
		case V4L2_PIX_FMT_NV12:
			*format = LINUX_CAMERA_FORMAT_NV12;
			break;
		case V4L2_PIX_FMT_NV21:
			*format = LINUX_CAMERA_FORMAT_NV21;
			break;
		case V4L2_PIX_FMT_YUV420:
			*format = LINUX_CAMERA_FORMAT_YU12;
			break;
		case V4L2_PIX_FMT_YVU420:
			*format = LINUX_CAMERA_FORMAT_YV12;
			break;
		case V4L2_PIX_FMT_MJPEG:
			*format = LINUX_CAMERA_FORMAT_MJPG;
			break;
		default:
			*format = LINUX_CAMERA_FORMAT_YUYV;
			break;
	...
	ctx->buf = calloc(req.count, sizeof(struct linux_cam_buf_t));
	if(!ctx->buf)
	{
		close(ctx->fd);
		free(ctx);
...
{
	struct linux_cam_context_t * ctx = (struct linux_cam_context_t *)context;
	enum v4l2_buf_type type;
	int i;
