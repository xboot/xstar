#ifndef __XSTAR_LIBX_LOG2_H__
#define __XSTAR_LIBX_LOG2_H__
...
static inline __attribute__((always_inline)) int xfls64(uint64_t x)
{
	uint32_t h = x >> 32;
	if (h)
		return xfls(h) + 32;
...
static inline __attribute__((always_inline)) unsigned xfls_long(unsigned long l)
{
	if(sizeof(l) == 4)
		return xfls(l);
	return xfls64(l);
