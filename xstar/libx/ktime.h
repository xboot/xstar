#ifndef __XSTAR_LIBX_KTIME_H__
#define __XSTAR_LIBX_KTIME_H__
...
static inline ktime_t ktime_set(const int64_t s, const unsigned long ns)
{
	if(s >= KTIME_SEC_MAX)
		return (ktime_t) { .tv64 = KTIME_MAX };
	return (ktime_t) { .tv64 = (int64_t)s * 1000000000ULL + (int64_t)ns };
}
...
static inline ktime_t ktime_add_us(const ktime_t kt, const uint64_t us)
{
	return ktime_add_ns(kt, us * 1000ULL);
}

static inline ktime_t ktime_sub_us(const ktime_t kt, const uint64_t us)
{
	return ktime_sub_ns(kt, us * 1000ULL);
}

static inline ktime_t ktime_add_ms(const ktime_t kt, const uint64_t ms)
{
	return ktime_add_ns(kt, ms * 1000000ULL);
}

static inline ktime_t ktime_sub_ms(const ktime_t kt, const uint64_t ms)
{
	return ktime_sub_ns(kt, ms * 1000000ULL);
}
...
static inline int64_t ktime_to_us(const ktime_t kt)
{
...
static inline int64_t ktime_us_delta(const ktime_t later, const ktime_t earlier)
{
	return ktime_to_us(ktime_sub(later, earlier));
}

static inline int64_t ktime_ms_delta(const ktime_t later, const ktime_t earlier)
{
