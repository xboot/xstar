#ifndef __XSTAR_DRIVER_CAMERA_H__
#define __XSTAR_DRIVER_CAMERA_H__
...
struct camera_t {
	...
	int (*start)(struct camera_t * cam, enum camera_format_t fmt, int width, int height);
	int (*stop)(struct camera_t * cam);
	int (*capture)(struct camera_t * cam, struct camera_frame_t * frame);
...
{
	exposure = XCLAMP(exposure, -1000, 1000);
...
{
	saturation = XCLAMP(saturation, -1000, 1000);
...
{
	brightness = XCLAMP(brightness, -1000, 1000);
...
{
	contrast = XCLAMP(contrast, -1000, 1000);
...
{
	sharpness = XCLAMP(sharpness, -1000, 1000);
...
#endif /* __XSTAR_DRIVER_CAMERA_H__ */
